...
class MapLoader {
	...
	static async loadLayers(map, name, gridSelector) {
		return this.loadMapJson(name)
			.then(data => {
                ...
                const { parallaxLayers, gridLayers, allLayers } = this.createLayers(data, projection);

				map.setLayers(allLayers);
                
                if (gridSelector != null) {
                    gridSelector.setGridLayers(gridLayers);
                }
			});
	}
	...
	static createLayers(data, projection) {
		...
		for(const gridLayer of data.grids)
		{
            ...
            mapLayer.set('gridId', gridLayer.gridId);
            mapLayer.set('gridName', gridLayer.name || `Grid ${gridLayer.gridId}`);
		...
		return {
            parallaxLayers,
            gridLayers,
            allLayers: [...parallaxLayers, ...gridLayers]
        };
	}
}
